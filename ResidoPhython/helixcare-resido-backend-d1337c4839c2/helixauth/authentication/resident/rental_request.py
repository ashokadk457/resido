...
class ResidentRentalRequestAuthentication(BaseResidentAuthentication):
    def authenticate(self, request):
        ...
        rental_request_form = self.check_sub_token_type(
            unverified_payload, required_sub_token_type="rental_request_form"
        )
        if not rental_request_form:
            raise InvalidToken(_("Token is not rental_request_form token"))
