...
from residents.models import ResidentRegisteredDevice


class ResidentRegisteredDeviceManager(BaseModelManager):
    model = ResidentRegisteredDevice

    def check_device_detail_and_store_on_login(self, device_detail):
        from residents.serializers import PatientRegisteredDeviceSerializer

        device_detail_serializer = PatientRegisteredDeviceSerializer(data=device_detail)
