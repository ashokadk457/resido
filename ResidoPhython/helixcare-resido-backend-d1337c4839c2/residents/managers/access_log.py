...
from residents.models import ResidentAccessLog
from residents.managers.registered_device import ResidentRegisteredDeviceManager


class ResidentAccessLogManager(BaseModelManager, BaseAccessLogManager):
    ...
    def log_user_access_from_device(
        ...
        exp_obj = None
        if refresh_token:
            jti, exp_obj = self.get_details_from_refresh_token(
                refresh_token=refresh_token
            )
        elif exp:
            exp_obj = datetime.datetime.fromtimestamp(exp)
        ...
        ResidentRegisteredDeviceManager().update_last_ip_and_location_on_device(
    ...
    def get_all_active_tokens_for_device(self, user, device):
        current_time = datetime.datetime.now()
        return self.filter_by(
            user=user, device=device, refresh_exp__gt=current_time
